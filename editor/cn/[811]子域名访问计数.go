...
func parse(str string) (count int, domains []string) {
	raws := strings.Split(str, " ")
	count, _ = strconv.Atoi(raws[0])
	domains = append(domains, raws[1])

	splitDomain := strings.Split(raws[1], ".")
	for ;len(splitDomain) != 1; {
		domain := strings.Join(splitDomain[1:], ".")
